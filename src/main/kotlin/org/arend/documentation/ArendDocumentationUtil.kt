...
const val EXTRA_AREND_DIR_HTML = ".extra-files/"
...
fun generateHtmlForArendLib(
    ...
    val service = psiProject.service<TypeCheckingService>()
    service.initialize()
    val moduleManager = ModuleManager.getInstance(psiProject)
    for (module in moduleManager.modules) {
        val config = ArendModuleConfigService.getInstance(module) ?: continue
        config.copyFromYAML(false)
        refreshLibrariesDirectory(psiProject.service<ArendProjectSettings>().librariesRoot)
        runReadAction {
            service.libraryManager.loadLibrary(config.library, ArendTypechecking.create(psiProject))
        }
    }
    ...
    val extraPath = basePath + File.pathSeparator + AREND_DIR_HTML + EXTRA_AREND_DIR_HTML
    ...
    while (extraFiles.isNotEmpty()) {
        ...
        ApplicationManager.getApplication().invokeLater {
            LocalFileSystem.getInstance().refreshAndFindFileByPath(extraAbsolutePath)?.let { visitChildrenRecursively(it, virtualFileVisitor) }
        }
...
fun generateHtmlForArend(
    ...
    val relativePathToCurDir = File(psiFile.containingDirectory.virtualFile.path).toRelativeString(File(projectBasePath)) + File.separator
            ...
            wrapTag("body") {
                ...
                psiFile.accept(object : PsiRecursiveElementVisitor() {
                    override fun visitElement(element: PsiElement) {
                        ...
                        val cssClass = ArendSyntaxHighlighter.map(elementType)

                        val href = if (element is ArendRefIdentifier) {
                            when (val resolve = element.resolve) {
                                is PsiFile -> {
                                    val relativePathToRefFile = if (resolve.containingFile.virtualFile is LightVirtualFile) {
                                        EXTRA_AREND_DIR_HTML + resolve.virtualFile.path.removePrefix("/").removeSuffix(AREND_EXTENSION)
                                    } else {
                                        File(resolve.virtualFile.path).toRelativeString(File(projectBasePath)).removeSuffix(AREND_EXTENSION)
                                    }

                                    "$host:$port/$projectName/$relativePathToRefFile$HTML_EXTENSION"
                                }
                                is PsiLocatedReferable -> {
                                    ...
                                    val result = if (resolveVirtualFile is LightVirtualFile) {
                                        ...
                                        getIdAndCounter(resolve, psiElementIds, counter, EXTRA_AREND_DIR_HTML + resolveVirtualFile.path.removeSuffix(AREND_EXTENSION).removePrefix("/"))
                                    } else {
                                        getIdAndCounter(resolve, psiElementIds, counter)
                                    }
                                    val id = result.first
                                    counter = result.second
                                    val relativePathToElementDir = result.third

                                    "$host:$port/$projectName/$relativePathToElementDir$HTML_EXTENSION#$id"
                                }
                                else -> null
                            }
                        } else {
                        ...
                        val result = getIdAndCounter(element, psiElementIds, counter)
                        val id = result.first
                        counter = result.second

                        append("<a${" id=\"$id\""}${href?.let { " href=\"$it\" style=\"text-decoration:none;\"" } ?: ""}${cssClass?.let { " class=\"$it\"" } ?: ""}>${StringEscapeUtils.escapeHtml(element.text)}</a>")
                    }
                })
...
private fun createColorCssFile(scheme: EditorColorsScheme) {
    ...
    for (arendColors in AREND_COLORS) {
        cssFile.appendText(".Arend .${arendColors.name} { color: ${String.format("#%06x", (scheme.getAttributes(arendColors.textAttributesKey)?.foregroundColor?.rgb ?: 0) and 0xFFFFFF)} }\n")
    }
    cssFile.appendText(".Arend a { text-decoration: none }\n")
    cssFile.appendText(".Arend a[href]:hover { background-color: darkseagreen }\n")
    cssFile.appendText(".Arend [href].hover-highlight { background-color: darkseagreen; }")
}

private fun getIdAndCounter(element: PsiElement, psiIds: MutableMap<String, Int>, counter: Int, relativePath: String? = null): Triple<Int, Int, String> {
    val relativePathToElementDir = relativePath ?: File(element.containingFile.virtualFile.path)
        .toRelativeString(File(element.project.basePath!!)).removeSuffix(AREND_EXTENSION)
    val key = relativePathToElementDir + ':' + element.textOffset.toString()
    val id = psiIds[key] ?: run {
        psiIds[key] = counter
        psiIds[key]!!
    }
    return Triple(id, counter + 1, relativePathToElementDir)
}
